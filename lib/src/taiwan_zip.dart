...
class TaiwanZip {
  ...
  TaiwanZip._(); // coverage:ignore-line

  /// Returns all the zip codes.
  static List<String> get zipCodes => mapping.values.toSet().toList();

  /// Returns all the cities.
  static List<String> get cities =>
      mapping.keys.map((value) => value.substring(0, 3)).toSet().toList();

  /// Returns the corresponding district based on zip code.
  ///
  /// For some cities,the zip code maps to multiple districts. In such cases
  /// the districts are separated with '/'. For example, when the input is 300,
  /// the output is '新竹市北區/新竹市東區/新竹市香山區'.
  static String toDistrict(String zipCode) {
    var districts = [];

    if (!mapping.containsValue(zipCode)) {
      throw DistrictNotFoundFailure(zipCode);
    }

    for (var entry in mapping.entries) {
      if (entry.value == zipCode) {
  ...
  static List<String> getDistricts(String city, {bool withCity = false}) {
    ...
    final filteredDistricts =
        mapping.keys.where((value) => value.contains(city));
